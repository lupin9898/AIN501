...
def _ensure_collection(client, collection_name: str, vector_size: int):
...
def run(
    ...
    # 1. Load
    log(f"Loading documents from {source.name} ...")
    ...
    _ensure_collection(retriever.client, settings.COLLECTION_NAME, vector_size)
    ...
    for i in range(0, len(points), batch_size):
        ...
        log(f"Upserted {min(i + batch_size, len(points))}/{len(points)} chunks ...")

    log(f"Done! {len(points)} chunks indexed.")
    return {
